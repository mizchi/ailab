...
function getMode(): SnapshotMode {
  ...
  // const postDoubleSlash = Deno.args.indexOf("--");
  // const postArgs = postDoubleSlash === -1
  //   ? Deno.args
  //   : Deno.args.slice(postDoubleSlash + 1);
...
import { printImage } from "./imgcat.ts";
...
export async function assertScreenshot(
  ...
  if (
    !existsSync(snapshotFilePath) || mode === "update" || options.updateSnapshot
  ) {
    // スナップショットを保存
 ...
 * ブラウザを使用するためのヘルパー関数
 */
export async function useBrowser(options: pptr.LaunchOptions = {}) {
